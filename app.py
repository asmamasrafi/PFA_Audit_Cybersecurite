...
+ questionnaire + résultat + génération de rapport PDF, en Streamlit.
"""
import io
...
div.stButton > button[kind="primary"]:hover {
    ...
    /* Bouton de téléchargement (aligné visuellement sur le style du site) */
...
# =====================================================================
def calculer_top_recommandations(reponses, limite=3):
    """
    Retourne les 'limite' recommandations prioritaires (index de question,
    axe, texte) en se basant sur les réponses les plus faibles (0 ou 1)
    parmi les questions notées.
    """
    faibles = sorted(reponses.items(), key=lambda x: x[1])
    top_faibles = [i for i, val in faibles if i in INDICES_NOTEES and val <= 1][:limite]
    resultats = []
    for i in top_faibles:
        resultats.append({
            "axe": QUESTIONS_FLAT[i]["axe"],
            "reco": RECOMMANDATIONS.get(i, "Renforcez cette bonne pratique."),
        })
    return resultats


# ---------- Palette sobre, façon document académique / LaTeX ----------
PDF_INK = colors.HexColor("#1A1A1A")
PDF_GRAY = colors.HexColor("#595959")
PDF_RULE_LIGHT = colors.HexColor("#BFBFBF")

# Une seule teinte discrète par niveau, utilisée uniquement sur le mot du
# niveau (pas de fond coloré, pas de badge)
PDF_NIVEAU_HEX = {
    "Niveau 1 — Initial": "#7A2E2E",
    "Niveau 2 — Basique": "#7A5A2E",
    "Niveau 3 — Intermédiaire": "#2E4A7A",
    "Niveau 4 — Avancé": "#2E6B44",
}


def _entete_pied_page(canvas_obj, doc_obj, nom_entreprise=""):
    """En-tête/pied de page sobres façon fancyhdr LaTeX : texte + filet fin."""
    ...
    canvas_obj.drawString(2.4 * cm, hauteur - 1.5 * cm, "CMRPI — Espace Maroc Cyberconfiance")
    canvas_obj.drawRightString(
        largeur - 2.4 * cm, hauteur - 1.5 * cm,
        nom_entreprise if nom_entreprise else "Rapport d'audit cybersécurité"
    )
    canvas_obj.setStrokeColor(PDF_RULE_LIGHT)
    canvas_obj.setLineWidth(0.5)
    canvas_obj.line(2.4 * cm, hauteur - 1.65 * cm, largeur - 2.4 * cm, hauteur - 1.65 * cm)

    # Pied de page
    canvas_obj.line(2.4 * cm, 1.75 * cm, largeur - 2.4 * cm, 1.75 * cm)
    canvas_obj.setFont("Times-Roman", 9)
    canvas_obj.setFillColor(PDF_GRAY)
    canvas_obj.drawCentredString(largeur / 2, 1.35 * cm, f"— {doc_obj.page} —")

    canvas_obj.restoreState()


def generer_pdf_rapport(dernier: dict, nom_entreprise: str = "") -> bytes:
    """
    Génère un rapport PDF (en mémoire), mise en page sobre façon document
    académique : police serif, filets fins, tableau type "booktabs" pour
    le détail par thème, liste numérotée pour les recommandations — pas de
    blocs colorés ni de badges. Retourne le PDF sous forme de bytes.
    """
    score = dernier["score"]
    score_max = dernier["score_max"]
    niveau_nom = dernier["niveau"]
    par_theme = dernier["par_theme"]
    reponses = dernier["reponses"]
    ...
    style_titre = ParagraphStyle(
        "TitrePrincipal", parent=styles["Title"], fontName="Times-Bold",
        fontSize=19, textColor=PDF_INK, alignment=1, spaceAfter=6, leading=23
    )
    style_sous_titre = ParagraphStyle(
        "SousTitre", parent=styles["Normal"], fontName="Times-Italic",
        fontSize=11, textColor=PDF_GRAY, alignment=1, spaceAfter=2
    )
    style_meta = ParagraphStyle(
        "Meta", parent=styles["Normal"], fontName="Times-Roman",
        fontSize=9.5, textColor=PDF_GRAY, alignment=1, spaceAfter=2
    )
    style_h2 = ParagraphStyle(
        "H2", parent=styles["Normal"], fontName="Times-Bold", fontSize=13,
        textColor=PDF_INK, spaceBefore=18, spaceAfter=8
    )
    style_corps = ParagraphStyle(
        "Corps", parent=styles["Normal"], fontName="Times-Roman",
        fontSize=10.5, leading=15, textColor=PDF_INK, spaceAfter=6
    )
    style_table_entete = ParagraphStyle(
        "TableEntete", parent=styles["Normal"], fontName="Times-Bold",
        fontSize=10, textColor=PDF_INK
    )
    style_table_cellule = ParagraphStyle(
        "TableCellule", parent=styles["Normal"], fontName="Times-Roman",
        fontSize=10, textColor=PDF_INK
    )
    style_table_chiffre = ParagraphStyle(
        "TableChiffre", parent=styles["Normal"], fontName="Times-Roman",
        fontSize=10, textColor=PDF_INK, alignment=1
    )
    style_reco = ParagraphStyle(
        "Reco", parent=styles["Normal"], fontName="Times-Roman",
        fontSize=10.5, leading=15.5, textColor=PDF_INK, spaceAfter=9,
        leftIndent=14, firstLineIndent=-14
    )
    style_note = ParagraphStyle(
        "Note", parent=styles["Normal"], fontName="Times-Italic",
        fontSize=8.5, leading=12, textColor=PDF_GRAY
    )

    couleur_niveau_hex = PDF_NIVEAU_HEX.get(niveau_nom, "#1A1A1A")

    elements = []

    # ---------- Bloc titre (façon \maketitle) ----------
    elements.append(Paragraph("Rapport d'audit de maturité cybersécurité", style_titre))
    elements.append(Paragraph(
        "Évaluation indicative — Guide de bonnes pratiques CMRPI/AUSIM", style_sous_titre
    ))
    meta = f"{nom_entreprise + ' &middot; ' if nom_entreprise else ''}{dernier['date']}"
    elements.append(Paragraph(meta, style_meta))
    elements.append(Spacer(1, 4))
    elements.append(HRFlowable(width="100%", thickness=0.75, color=PDF_INK, spaceBefore=10, spaceAfter=18))

    # ---------- 1. Score global ----------
    elements.append(Paragraph("1. Score de maturité globale", style_h2))
    ratio_global = score / score_max if score_max else 0
    elements.append(Paragraph(
        f"L'entreprise obtient un score de <b>{score} sur {score_max}</b> "
        f"({ratio_global * 100:.0f}&nbsp;% des bonnes pratiques évaluées), correspondant au "
        f"<font color='{couleur_niveau_hex}'><b>{niveau_nom}</b></font>.",
        style_corps
    ))

    # ---------- 2. Détail par thème (tableau type "booktabs") ----------
    elements.append(Paragraph("2. Détail par thème", style_h2))

    lignes = [[
        Paragraph("Thème", style_table_entete),
        Paragraph("Score", style_table_entete),
        Paragraph("Taux", style_table_entete),
    ]]
    for nom_axe, (s, m) in par_theme.items():
        ratio = s / m if m else 0
        lignes.append([
            Paragraph(nom_axe, style_table_cellule),
            Paragraph(f"{s} / {m}", style_table_chiffre),
            Paragraph(f"{ratio * 100:.0f}&nbsp;%", style_table_chiffre),
        ])

    table_themes = Table(lignes, colWidths=[10.5 * cm, 2.6 * cm, 2.6 * cm])
    table_themes.setStyle(TableStyle([
        ("LINEABOVE", (0, 0), (-1, 0), 1, PDF_INK),
        ("LINEBELOW", (0, 0), (-1, 0), 0.5, PDF_INK),
        ("LINEBELOW", (0, -1), (-1, -1), 1, PDF_INK),
    ...
    elements.append(Paragraph("3. Recommandations prioritaires", style_h2))
    top_recos = calculer_top_recommandations(reponses, limite=3)
    if not top_recos:
        elements.append(Paragraph(
            "Aucune faiblesse majeure n'a été détectée sur ce questionnaire. "
            "Il est recommandé de maintenir les bonnes pratiques actuelles.",
            style_corps
        ))
    else:
        for rang, item in enumerate(top_recos, start=1):
            elements.append(Paragraph(
                f"{rang}.&nbsp;&nbsp;<b>{item['axe']}</b> — {item['reco']}",
                style_reco
            ))

    elements.append(Spacer(1, 14))
    elements.append(HRFlowable(width="100%", thickness=0.5, color=PDF_RULE_LIGHT, spaceAfter=8))
    elements.append(Paragraph(
        "Ce rapport est généré automatiquement à partir des réponses fournies et propose des "
        "pistes générales d'amélioration. Il ne remplace pas un audit de sécurité approfondi "
        "réalisé par un professionnel.",
        style_note
    ))

    dessin_page = partial(_entete_pied_page, nom_entreprise=nom_entreprise)
    doc.build(elements, onFirstPage=dessin_page, onLaterPages=dessin_page)
...
def page_resultat():
    ...
    top_recos = calculer_top_recommandations(dernier["reponses"], limite=3)
    if not top_recos:
        st.success("Bravo, aucune faiblesse majeure détectée sur ce questionnaire !")
    else:
        for rang, item in enumerate(top_recos, start=1):
            st.markdown(
                f'<div class="reco-box"><div class="reco-theme">{rang}. {item["axe"]}</div>{item["reco"]}</div>',
                unsafe_allow_html=True,
            )
    ...
    nom_entreprise = st.text_input(
        "Nom de l'entreprise (optionnel, apparaîtra sur le rapport)", value=""
    )
    pdf_bytes = generer_pdf_rapport(dernier, nom_entreprise)

    st.write("")
    col_retour, col_pdf = st.columns([1, 1])
    with col_retour:
